...
import { Router } from '@angular/router'; // Add this line to import Router
...
export class CustomerMenuComponent implements OnInit {
  // Example menu items (same as before)
  menuItems = [
    ...
    // Filter menu items based on search term, filters, and country
    filterMenu() {
    ...
    removeFromCart(item: any) {
      ...
      if (index > -1) {
        ...
        this.total -= item.price; // Update the total
      }
    }
    ...
    confirmPayment() {
      const paymentDetails = {
        userId: 1, // This should come from the logged-in user
        amount: this.total,
      ...
      this.http.post('http://localhost:5000/payments', paymentDetails)
        .subscribe((response: any) => {
          ...
          this.generateBill(); // Generate the bill after payment confirmation
        }, (error) => {
    ...
    generateBill() {
      ...
      alert(billDetails);  // This can be replaced with a more advanced printout or invoice
    }
  }